...
wait_ready iso-api-orchestrator
wait_ready iso-web

api_host="$(oc get route iso-api-orchestrator -n "${NS}" -o jsonpath='{.spec.host}' 2>/dev/null || true)"
...
if [[ -n "${api_host}" ]]; then
  ...
  [[ "${code}" == "200" ]] || fail "iso-api-orchestrator /health returned ${code}"
fi
